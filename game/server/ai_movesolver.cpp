...
inline float CAI_MoveSolver::round( float f )
{
...
bool CAI_MoveSolver::Solve( const AI_MoveSuggestion_t *pSuggestions, int nSuggestions, AI_MoveSolution_t *pResult)
{
	...
	for ( int iSuggestion = 0; iSuggestion < suggestions.Count(); ++iSuggestion )
	{
		...
		int   center 	= CAI_MoveSolver::round( ( halfSpan * NUM_SOLUTIONS ) / 360 ); // 2838: have to redirect to the actual round function due to conflicts with vs2013 compiler
