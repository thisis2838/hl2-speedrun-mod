...
void CServerGameDLL::GameFrame(bool simulating)
{
	...
	if (SpeedrunTimer::timer()->IsRunning()) {
		if (SpeedrunTimer::timer()->InLevelLoad()) {
			CBasePlayer* player = UTIL_GetLocalPlayer();
			if (player) {
				//Msg("Found the player at tick %f!\n", (float) gpGlobals->tickcount);
				SpeedrunTimer::timer()->Init((float)gpGlobals->tickcount);
				SpeedrunTimer::timer()->SetRunning(true);
			}
		}
		else {
...
void CServerGameDLL::LevelShutdown(void)
{
	...
	if (SpeedrunTimer::timer()->IsRunning()) {
		SpeedrunTimer::timer()->SetLevelLoad(true);
	}
